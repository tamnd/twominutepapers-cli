...
describes what users get. Pick a format with `-o`, or let twominutepapers choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
twominutepapers <command> -o table   # aligned columns for reading
twominutepapers <command> -o jsonl   # one JSON object per line, for piping
twominutepapers <command> -o json    # a single JSON array
twominutepapers <command> -o csv     # spreadsheet friendly
twominutepapers <command> -o tsv     # tab-separated
twominutepapers <command> -o url     # just the URL column
twominutepapers <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
twominutepapers <command> --fields id,title,url
```
...
```bash
twominutepapers <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
twominutepapers <command>            # a table, because this is a terminal
twominutepapers <command> | wc -l    # JSONL, because this is a pipe
```
